...
const SecurityContent = () => {
    ...
    const { settings, isLoading, updateSetting } = useSecurity();

    // Merge the static schema with the values loaded from the server
    const securitySettings = SECURITY_SETTINGS_SCHEMA.map((item) => ({
        ...item,
        status: item.type === 'switch' ? Boolean(settings?.[item.id]) : null,
        currentOption: item.type === 'dropdown' ? settings?.[item.id] || item.options?.[0] : undefined,
    }));
