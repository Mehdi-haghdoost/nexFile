...
const FolderCard = ({ folder, folderName, onMenuClick, onClick }) => {
    // Accept either a full folder object or just a name, so existing
    // callers keep working while the menu needs the folder's id.
    const resolvedFolder = folder || (folderName ? { name: folderName } : null);
    const displayName = resolvedFolder?.name || 'Untitled';
    const hasId = Boolean(resolvedFolder?.id);
    ...
    // Fall back to the legacy menu callback when no folder id is available
    const handleLegacyMenuClick = (e) => {
        e.stopPropagation();
        if (onMenuClick) {
            onMenuClick(displayName);
        }
    };
    ...
    return (
        ...
        >
            ...
            <div className='relative shrink-0'>
                {hasId ? (
                    <FolderActionMenu folder={resolvedFolder} />
                ) : (
                    <svg
