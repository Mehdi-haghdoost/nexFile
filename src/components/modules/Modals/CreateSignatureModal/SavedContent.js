        ...
        <article className='flex flex-col items-start gap-6 self-stretch'>
            <header className='flex flex-col items-center gap-4 self-stretch'>
                ...
                <section className='flex flex-col items-center gap-1'>
                    <h2 className='text-medium-16 text-neutral-500'>Saved Signatures</h2>
            ...
            <main className='flex flex-col items-start gap-4 self-stretch'>
                <header className='flex items-center justify-between self-stretch'>
                    <h3 className='text-medium-16 text-neutral-600'>Your Signatures</h3>
                    <aside className='text-regular-12 text-neutral-400'>
                ...
                {savedSignatures.length > 0 ? (
                    <section className='flex flex-col gap-3 self-stretch' role="list">
                        {savedSignatures.map((signature) => (
                            <article 
                                key={signature.id} 
                                className='flex items-center justify-between p-3 rounded-lg border border-stroke-200 bg-white hover:bg-gray-50 transition-all'
                                role="listitem"
                            >
                                <section 
                                ...
                                >
                                    <figure className='w-12 h-8 bg-gray-100 rounded border flex items-center justify-center'>
                                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
                                            <path d="M2 13C5.333 11.5 7.5 9.5 7.5 7.5C7.5 5.5 6.5 5.5 5.5 5.5C4.5 5.5 3.5 6.25 3.53 7.5C3.56 8.82 4.74 9.36 5.25 10.5C6.25 12 6.75 12.5 7.5 11.5C8.17 10.5 8.67 9.67 9 9C9.75 11.5 11 12.5 12.5 12.5H14.5M14.5 12.5L12.5 10V1.5C12.5 0.948 12.948 0.5 13.5 0.5C14.052 0.5 14.5 0.948 14.5 1.5V10L14.5 12.5ZM12.5 3.5H14.5" stroke="#6B7280" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round"/>
                                        </svg>
                                    </figure>
                                    <section className='flex-1'>
                                        <header className='flex items-center gap-2'>
                                            <h4 className='text-medium-14 text-neutral-600'>{signature.name}</h4>
                                            <span className='text-regular-10 text-neutral-400 bg-gray-100 px-2 py-0.5 rounded'>
                                                {signature.type}
                                            </span>
                                        </header>
                                        <time className='text-regular-12 text-neutral-400'>
                                ...
                                <aside className='flex items-center gap-2'>
                                    <button 
                                        onClick={() => handleSelectSignature(signature)}
                                        type="button"
                                        className='p-2 hover:bg-gray-100 rounded text-regular-12 text-primary-500'
