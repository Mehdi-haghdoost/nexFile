  ...
  return (
    <section className='flex flex-col items-start gap-2 self-stretch' ref={dropdownRef}>
      ...
      <div className='flex flex-col self-stretch relative'>
        ...
        <div className='flex items-center justify-between gap-3 h-[52px] py-3 px-4 rounded-lg border border-stroke-500 dark:border-neutral-600 bg-white dark:bg-neutral-800 hover:bg-gray-50 transition-colors cursor-pointer'>
          <div className='flex items-center gap-3'>
           <FolderIcon staticColor={true} />
            <div className='flex items-center gap-2'>
              <span className='text-medium-14 dark:text-medium-14-white'>{folderName}</span>
              {ownerName !== 'No Owner' && (
                <>
          ...
          <button
            ...
            disabled={!hasFolders}
            className={`text-right text-medium-12-primary transition-colors flex items-center gap-1 ${hasFolders
        ...
        {isDropdownOpen && hasFolders && (
          <div className='absolute top-full z-20 w-full mt-2 bg-white dark:bg-neutral-900 rounded-lg shadow-lg border border-gray-200 dark:border-dark-border max-h-60 custom-scrollbar'>
            {folders.map((folder, index) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center w-full text-left py-3 px-4 dark:border-neutral-700 hover:bg-gray-50 dark:hover:bg-neutral-800/50 transition-all duration-300 ease-[cubic-bezier(0.25,0.1,0.25,1)] hover:shadow-custom hover:-translate-y-0.5 cursor-pointer group ${isSelected ? 'bg-gray-50 dark:bg-neutral-800/50' : ''
                    }`}
                >
                  <FolderIcon />
