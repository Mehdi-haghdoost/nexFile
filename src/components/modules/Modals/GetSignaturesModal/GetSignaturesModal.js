...
const GetSignaturesModal = () => {
    ...
    // ✅ KEY FIX: فقط وقتی modal باز هست mount میشه
    // این باعث میشه useSignatures و useFiles فقط وقتی لازمه اجرا بشن
    if (!isOpen) return null;

    return <GetSignaturesModalInner closeModal={closeModal} />;
};

// Inner component جدا تا hooks فقط وقتی modal باز هست اجرا بشن
                ...
                <article className="w-full flex flex-col max-h-[80vh]">
                                    ...
                                    pdfFiles.map((pdf) => (
                                        ...
                                        >
                                            ...
                                            <div className='flex-1 min-w-0'>
                                                <h3 className='text-sm font-medium text-neutral-600 dark:text-white truncate'>{pdf.name}</h3>
                                                <p className='text-xs text-neutral-400 dark:text-neutral-300'>{(pdf.size / 1024 / 1024).toFixed(2)} MB</p>
                                            </div>
                                            {selectedPdf?.id === pdf.id && (
                                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                                ...
                                {signaturesLoading ? (
                                    ...
                                    signatures.map((signature) => (
                                        ...
                                        >
                                            ...
                                            <div className='flex-1 min-w-0'>
                                                <h3 className='text-sm font-medium text-neutral-600 dark:text-white truncate'>{signature.name}</h3>
                    ...
                    {step === 2 && (
                        <footer className="flex-shrink-0 flex justify-end gap-3 pt-4 sm:pt-6">
                            <button onClick={handleBack} disabled={isProcessing} type="button"
                            ...
                            <button onClick={handleApplySignature} disabled={!selectedSignature || isProcessing} type="button"
