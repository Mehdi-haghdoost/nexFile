...
import { SaveIcon, RotateLeftIcon } from '@/components/ui/icons';

// Fixed-position action dropdown for a content (folder) row
const ContentActionMenu = ({ item, onSetArchived }) => {
    ...
    const actions = item.isArchived
        ? [{ label: 'Unarchive', onClick: () => onSetArchived(item.id, false), icon: <RotateLeftIcon /> }]
        : [{ label: 'Archive', onClick: () => onSetArchived(item.id, true), icon: <SaveIcon /> }];
    ...
    return (
        <div className='flex justify-center items-center gap-2 w-[52px] py-0 px-3 flex-shrink-0'>
            <button
                ...
                aria-label={`Actions for ${item.folder}`}
                aria-haspopup="true"
                ...
                className={`flex items-center justify-center w-8 h-8 p-1 shadow-custom border rounded cursor-pointer hover:scale-105 active:scale-95 transition-all flex-shrink-0
                    ${isOpen
                        ...
                        : 'border-[#F2F2F3] dark:border-dark-border bg-white dark:bg-dark-gradient dark:shadow-dark-panel hover:bg-gray-50 dark:hover:bg-neutral-700'
                    }`}
            >
