...
import { CopyLinkIcon, EyeOffIcon, EyeOnIcon, RedTrashIcon } from '@/components/ui/icons';

// Self-contained action dropdown for a single member/invite row.
// Action set changes based on the row's status.
const MemberActionMenu = ({ member, onUpdateMember, onRemoveMember, onCancelInvite }) => {
    ...
    // The organization owner has no actions; they're always Admin
    if (member.isOwner) {
        return <span className='text-xs text-neutral-300 dark:text-neutral-400 px-2'>Owner</span>;
    }

    let actions = [];
    if (member.kind === 'invite') {
        actions = [
            { label: 'Copy invite link', onClick: handleCopyLink, icon: <CopyLinkIcon /> },
            { label: 'Cancel invite', onClick: () => onCancelInvite(member.id), icon: <RedTrashIcon />, danger: true },
        ];
    } else if (member.status === 'active') {
        actions = [
            { label: 'Suspend', onClick: () => onUpdateMember(member.id, { status: 'suspended' }), icon: <EyeOffIcon /> },
            { label: 'Remove', onClick: () => onRemoveMember(member.id), icon: <RedTrashIcon />, danger: true },
        ];
    } else if (member.status === 'suspended') {
        actions = [
            { label: 'Reactivate', onClick: () => onUpdateMember(member.id, { status: 'active' }), icon: <EyeOnIcon /> },
            { label: 'Remove', onClick: () => onRemoveMember(member.id), icon: <RedTrashIcon />, danger: true },
        ];
    } else if (member.status === 'removed') {
        actions = [
            { label: 'Restore', onClick: () => onUpdateMember(member.id, { status: 'active' }), icon: <EyeOnIcon /> },
        ];
    }
