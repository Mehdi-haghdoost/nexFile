...
const SettingsItem = ({ item, value, disabled, onToggle, onOpen }) => {
    ...
    // A toggle row must not also be clickable, or the switch would fire twice
    const Wrapper = isToggle ? 'div' : 'button';
    const wrapperProps = isToggle
        ? {}
        : { type: 'button', disabled, onClick: () => onOpen?.(item) };
        ...
        >
            ...
            {isToggle ? (
                <div className="relative z-10 flex items-center gap-2 flex-shrink-0">
                    <span className="hidden sm:inline text-xs sm:text-sm font-medium text-neutral-500 dark:text-white">
                        {value ? 'on' : 'off'}
                    </span>
                    <Switch
                        id={id}
