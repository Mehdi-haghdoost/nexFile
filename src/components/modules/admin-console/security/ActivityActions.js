...
const ActivityActions = ({ onClear, onExport, isExporting, hasActivity }) => {
    return (
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 w-full sm:w-auto">
            ...
            <button
                ...
                className="flex justify-center items-center gap-1.5 h-9 sm:h-8 py-1.5 sm:py-1 pr-3 sm:pr-4 pl-3 rounded-lg border border-error-400 bg-gradient-error shadow-light hover:shadow-middle transition-all duration-200 hover:brightness-110 active:scale-95 disabled:opacity-50 group w-full sm:w-auto"
            >
                <WhiteTrashIcon className="transition-transform duration-200 group-hover:scale-110 flex-shrink-0" />
                <span className="text-xs sm:text-sm font-medium text-white transition-colors duration-200">
                    Clear
                </span>
            </button>
        </div>
